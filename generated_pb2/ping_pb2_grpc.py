# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import ping_pb2 as ping__pb2


class PingServerStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.doPing = channel.unary_unary(
                '/ping.PingServer/doPing',
                request_serializer=ping__pb2.Ping.SerializeToString,
                response_deserializer=ping__pb2.Pong.FromString,
                )


class PingServerServicer(object):
    """Missing associated documentation comment in .proto file."""

    def doPing(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_PingServerServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'doPing': grpc.unary_unary_rpc_method_handler(
                    servicer.doPing,
                    request_deserializer=ping__pb2.Ping.FromString,
                    response_serializer=ping__pb2.Pong.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'ping.PingServer', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class PingServer(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def doPing(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/ping.PingServer/doPing',
            ping__pb2.Ping.SerializeToString,
            ping__pb2.Pong.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
